 ...
 */
export function getHSLWithRandomHue(hue:number, saturate: IntRange<0, 100> = 50, light: IntRange<0, 100> = 50): HSLColor {
    if(hue > 360 || hue < 0 || saturate > 100 || saturate < 0 || light > 100 || light < 0) return {
        h: 100,
        s: 50,
        l: 50
    }
...
export const hslToHslString = ( {h, s, l}: HSLColor): string => `hsl(${h}, ${s}%, ${l}%)`;
